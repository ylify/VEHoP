...
matrix = {}
new_matrix = {}
os.makedirs(pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/filtered')
out_fas = open(pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/supermatrix.new.fas','w')
out_partition = open(pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/partition_data.new.txt','w')
with open(pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/supermatrix.fas') as file:
...
partitions = {}
duplicated = 0
with open(pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/partition_data.txt') as file:
    for line in file:
        line = line.strip('\n')
        ...
        if OG_name not in partitions.keys():
            ...
            tmp_OG = open(pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/filtered/'+OG_name+'_pruned.fa','w')
...
out_log.flush()
try:
    if os.path.getsize(pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/partition_data.new.txt.contree') < 10:
        raise Exception
except:
...
os.system('ln -s '+pwd+'/'+prefix+'.Phylogenomics/phylopypruner_output/partition_data.new.txt.full.contree '+pwd+'/'+prefix+'.IQTREE2.full.tre')
end_time = time.time()
spend = end_time - start_time
